...
const SidebarHeader = ({
  ...
  <div
    className={`relative flex items-center w-full ${
      isMobile ? "h-16 mb-2" : "h-20 mt-4 mb-3"
    }`}
  >
    <div
    ...
    {!isMobile && (
      ...
      >
        {open
          ? React.createElement(iconMap["ChevronLeft"], {
              className: "text-blue-500 w-4 h-4 md:w-5 md:h-5",
            })
          : React.createElement(iconMap["ChevronRight"], {
              className: "text-blue-500 w-4 h-4 md:w-5 md:h-5",
            })}
      </button>
    )}
            ...
            {filterCategories.map(
              (section) =>
                expandedSection === section.heading && (
                  <ul
                    key={section.heading}
                    className="mt-1 ml-3 space-y-1 border-l-2 border-pink-500 pl-3"
                  >
                    {section.items.map((item) => (
                      <li key={item.label}>
                        <button
...
const SchemeCard = ({
  ...
  return (
    <div
      className={`rounded-xl p-4 md:p-6 transition-all duration-400 hover:shadow-xl border ${
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex-1">
            ...
            >
              {SchemeIcon
                ? React.createElement(SchemeIcon, {
                    className: `w-5 h-5 ${
                      isDark ? `text-${darkAccent}` : `text-${lightAccent}`
                    }`,
                  })
                : null}
            </div>
          ...
          <div
            className={`text-sm flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 ${
...
const WelfareSchemesPage: React.FC<WelfareSchemesProps> = ({
  ...
  const displayedSchemes = schemes.filter((scheme) => {
    ...
    const matchesYears =
      ...
      selectedYears.includes(scheme.launchedYear);
    ...
    >
      ...
      >
        <div className="p-4 md:p-6">
          ...
          <div
            className={`rounded-xl md:rounded-2xl p-4 md:p-6 mb-4 md:mb-6 shadow-xl ${
          ...
          >
            ...
            >
              {/* filters body (category/year) stays as you wrote it */}
            </div>
